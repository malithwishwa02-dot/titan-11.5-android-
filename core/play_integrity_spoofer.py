...
class PlayIntegritySpoofer:
    ...
    def _inject_keybox(self) -> bool:
        """Inject keybox for hardware attestation.

        Priority:
          1. External real keybox at /opt/titan/data/keybox/keybox.xml (from stock ROM dump)
          2. Minimal placeholder keybox (sufficient for DEVICE tier with PIF module)

        For STRONG tier, use _configure_rka_proxy() instead. The placeholder keybox
        does NOT contain real EC/RSA private key material and won't pass hardware
        attestation on its own — it only provides the XML structure TrickyStore expects.
        """
        keybox_path = "/data/adb/tricky_store/keybox.xml"
        alt_path = "/data/misc/keybox/attestation_keybox"

        # Try to use real keybox from /opt/titan/data/ first
        external_keybox = "/opt/titan/data/keybox/keybox.xml"
        if os.path.exists(external_keybox):
            tmp_path = external_keybox
            logger.info("Using external real keybox from /opt/titan/data/keybox/")
        else:
            # Minimal placeholder — sufficient for DEVICE tier with PIF module
            keybox_xml = ('<?xml version="1.0"?>\n<AndroidAttestation>\n'
                          '  <NumberOfKeyboxes>1</NumberOfKeyboxes>\n'
                          '  <Keybox DeviceID="titan">\n'
                          '    <Key algorithm="ecdsa"><Format>pem</Format></Key>\n'
                          '  </Keybox>\n</AndroidAttestation>\n')
        ...
        adb_shell(self.target, "mkdir -p /data/adb/tricky_store /data/misc/keybox", timeout=5)

        ok1 = adb_push(self.target, tmp_path, keybox_path)
        ok2 = adb_push(self.target, tmp_path, alt_path)
        # Only delete if it was a temp file we created (not external)
        if tmp_path != external_keybox:
            os.unlink(tmp_path)

        if ok1 or ok2:
            adb_shell(self.target,
                f"chmod 600 {keybox_path} {alt_path} 2>/dev/null && "
                f"chown system:system {alt_path} 2>/dev/null",
                timeout=5)
            adb_shell(self.target, "setprop persist.titan.keybox.loaded 1", timeout=3)
            self._keybox_installed = True
            logger.info("Static keybox injected")
            return True
